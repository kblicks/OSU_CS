...
** Date: 8/14/2020
** Description: Hosts the main function to create a linked list and test the functions
*********************************************************************/
...
int count_prime(Linked_List &list)
{
    int size = list.get_length();
    int count = 0;
    //Loop to get each value in the list

    for (int i = 0; i < size; i++)
    {
        int prime = 0;
        int val = list.get_value(i);
        int num = val / 2;

        //get rid of negatives and 1
        if (val <= 1)
        {
            //Special case
        }
        else //Check the number to see if it's prime
        {
            for (int j = 2; j <= num; j++)
            {
                if (val % j == 0)
